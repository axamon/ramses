...
func CreaTrap(device, argomento, summary, ipdevice, ipserver, community string, port uint16, specific, severity int) (result *g.SnmpPacket, err error) {

	// In tutti gli altri casi si può inviare la trap

	// Default is a pointer to a GoSNMP struct that contains sensible defaults
	// eg port 161, community public, etc
	g.Default.Target = ipserver
	g.Default.Port = port
