...
func main() {
	...
	//numSamples := 500 //strconv.Atoi(os.Args[1])

	// Equation 3-10.
	x := func(n int) float64 {
		wave0 := 25.0 + (float64(n) / 150) + rand.Float64()*50
		//wave0 := 3*math.Sin(2*math.Pi*float64(n)/20.0) + 2.0 + float64(n)/10
		wave1 := math.Sin(2 * math.Pi * float64(n) / 30) //settimanale
		wave3 := math.Sin(2 * math.Pi * float64(n) / 20) //gionaliero
	...
	//fmt.Println(len(frequenze)) //debug
	...
	sinwave := func(n int, ampiezza, frequenza float64) float64 {
		wave := math.Sin(2 * math.Pi * frequenza * float64(n))
		//wave := math.Sin(* 2 * math.Pi * frequenza * float64(n))
	...
	//mediadiff := stat.Mean(diff, nil)
	for i := 0; i < finestranalisi; i++ {
		//p, f := cmplx.Polar(X[i])
		///= p * math.Exp(f*math.Sqrt(-1))
		...
		/*
			...
			//	sinwavediscrete10[i])
		*/
		stagionalita = append(stagionalita, sumsinewaves*stat.Mean(a, nil)+alpha)

		//	diff[i] = diff[i] - sumsinewaves*stat.Mean(a, nil)

	}

	//Trasformata di fourier dell'andamento originale senza trend
	FOriginale := fft.FFTReal(diff)

	//Trasformata di fourier dell'andamento stagionale
	FStagionale := fft.FFTReal(stagionalita)

	//Inseriamo le trasformate in mappe
	FOriginalemap := make([]complex128, numSamples)
	FStagionalemap := make([]complex128, numSamples)

	for f := 0; f < finestranalisi; f++ {
		FOriginalemap[f] = FOriginale[f]
		FStagionalemap[f] = FStagionale[f]
	}

	//Eliminando la stagionalità nel dominio delle frequenze
	//Si ottiene una mappa di numeri complessi
	var FDiff []complex128
	for l := 0; l < len(FOriginale); l++ {

		element := FOriginalemap[l] - FStagionalemap[l]
		//fmt.Println(FOriginale[l], FStagionale[l], FDiff)
		FDiff = append(FDiff, element)

	}

	//Effettuando la trasformata inversa inversa della differenza tra originale senza trend e stagionalità
	D := fft.IFFT(FDiff)
	var TDiff []float64
	//Trasformiamo i complessi in float64
	for _, complesso := range D {
		TDiff = append(TDiff, cmplx.Abs(complesso))
	}

	//diffmode, _ := stat.Mode(diff, nil)
	TDiffmean, TDiffstd := stat.MeanStdDev(TDiff, nil)

	for i := 0; i < len(TDiff); i++ {
		if math.Abs(TDiff[i]) < TDiffmean+2*TDiffstd {
			TDiff[i] = 0
			continue
		}
		//TDiff[i] = 1
	...
	*/
	...
	err = plotutil.AddLinePoints(p,
		...
		"Andamento senza trend", points(diff, numSamples),
		"Stagionalità", points(stagionalita, finestranalisi),
		//"SS", points(ss, numSamples),
		//"Fourier", points(l, numSamples),
		//
		"Anomalie", points(TDiff, numSamples),
	)
